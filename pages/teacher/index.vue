      ...
      <section class="c-sort-box unBr">
        <div>
          ...
          <article class="i-teacher-list" v-if="map.total > 0">
            <ul class="of">
              <li v-for="teacher in map.items" :key="teacher.id"> <!--:key表示每个li的唯一标识-->
        ...
        <div>
          <div class="paging">
            ...
            <a
              ...
              href="#"
              title="首页"
              @click.prevent="gotoPage(1)">首页</a><!--@click.prevent 表示组织a标签跳转，执行gotoPage方法-->
            <a
              :class="{undisable: !map.hasPrevious}"
              href="#"
              title="前一页"
              @click.prevent="gotoPage(map.current-1)">&lt;</a>
            <a
            ...
            <a
              ...
              href="#"
              title="末页"
              @click.prevent="gotoPage(map.pages)">末页</a>
            <div class="clear"/>
          </div>
        </div>
        <!-- 公共分页 结束 -->
...
export default {
  //异步调用,页面加载只调用一次
  //参数1：相当this.$route.params
  asyncData({ params, error }){
    return teacherApi.getTeacherList(1,8)
      .then(response =>{
        return {map : response.data.data}  //得到后台的map集合,相当于this.data = response.data.data
      })
  },
