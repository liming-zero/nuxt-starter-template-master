    ...
    <div class="mt50 commentHtml"><div>
      ...
      <section class="">
          <section class="question-list lh-bj-list pr">
            <ul class="pr10">
              <li v-for="comment in map.items" :key="comment.id">
                  <aside class="noter-pic">
        ...
        <div class="paging">
            ...
            href="#"
            title="末页"
            @click.prevent="gotoPage(map.pages)">末</a>
            <div class="clear"/>
        </div>
        <!-- 公共分页 结束 -->
      </div>
  ...
  methods:{
    ...
    initComment(){
       commentApi.getCommentList(this.page,this.limit,this.courseId)
        .then(response =>{
          this.map = response.data.data  //拿到map集合中的数据
        })
    },
    ...
    gotoPage(page){
      commentApi.getCommentList(page,this.limit,this.courseId)
        .then(response =>{
          this.map = response.data.data  //拿到map集合中的数据
        })
    }
